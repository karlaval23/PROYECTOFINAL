<?php 
	...
	class Tienda extends Controllers{
		...
		public function procesarVentaCulqi() {
			// Recibe los datos
			$token = $_POST['token'];
			$email = $_POST['email'];
			$total = $_POST['total'];
			...
			$SECRET_KEY = 'pk_test_3999d292a3cb5972'; // Reemplaza con tu llave secreta de Culqi
			...
			$data = array(
				...
				"antifraud_details" => array(
					"first_name" => "Nombre", // Puedes ajustar estos datos
					"last_name" => "Apellido",
			...
			if ($http_status == 201) {
				...
				$this->guardarTransaccion($response_data);
				echo json_encode(array("status" => true, "msg" => "Pago realizado con éxito"));
			} else {
				// La transacción falló
				echo json_encode(array("status" => false, "msg" => $response_data['user_message']));
			}
		}
		
		private function guardarTransaccion($data) {
			// Lógica para guardar la transacción en tu base de datos
			// Puedes guardar los detalles de $data en tu tabla de transacciones
		}
		...
		public function procesarVenta() {
			if ($_POST) {
				...
				if (!empty($_SESSION['arrCarrito'])) {
					...
					if ($tipopagoid == 6 && !empty($_POST['token'])) {
						$datospay = json_encode(array(
							"token" => $_POST['token'],
							"email" => $_POST['email']
						));
						$idtransaccion = $_POST['token'];
						$status = "Completo"; // Asumiendo aprobación inmediata para simplificación
		
						// Crear pedido
						$request_pedido = $this->insertPedido($idtransaccion, 
															  $datospay, 
															  $personaid,
															  $costo_envio,
															  $monto, 
						...
						if ($request_pedido > 0) {
							...
							// Cifrar datos sensibles para la sesión
						...
						// Manejo de otros métodos de pago como Contra Entrega o PayPal
						if ($tipopagoid == 1) { // Contra Entrega
							// Crear pedido
							$request_pedido = $this->insertPedido(NULL, 
																  NULL, 
																  $personaid,
																  $costo_envio,
																  $monto, 
						...
						} elseif ($tipopagoid == 2) { // PayPal
							if (!empty($_POST['datapay'])) {
								$jsonPaypal = $_POST['datapay'];
								$objPaypal = json_decode($jsonPaypal);
								$status = "Aprobado"; // Asumiendo aprobación inmediata para simplificación
								...
								if (is_object($objPaypal)) {
									...
									$arrResponse = array("status" => false, "msg" => 'Hubo un error en la transacción con PayPal.');
								}
							} else {
								$arrResponse = array("status" => false, "msg" => 'No se recibieron datos de PayPal.');
							}
						} else {
							$arrResponse = array("status" => false, "msg" => 'Método de pago no válido.');
						}
					}
				} else {
					$arrResponse = array("status" => false, "msg" => 'No es posible procesar el pedido.');
