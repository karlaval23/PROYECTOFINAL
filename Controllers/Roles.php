		...
		public function getRoles()
{
    if ($_SESSION['permisosMod']['r']) {
        ...
        for ($i = 0; $i < count($arrData); $i++) {
            ...
            // Agregar este bloque para la columna "Observaciones"
            $arrData[$i]['observaciones'] = isset($arrData[$i]['observaciones']) ? $arrData[$i]['observaciones'] : '';
		...
		public function setRol()
    {
        ...
        if ($intIdrol == 0) {
            ...
            if ($_SESSION['permisosMod']['w']) {
                $request_rol = $this->model->insertRol($strRol, $strDescripcion, $strObservaciones, $intStatus);
                $option = 1;
            }
        } else {
            //Actualizar
            if ($_SESSION['permisosMod']['u']) {
                $request_rol = $this->model->updateRol($intIdrol, $strRol, $strDescripcion, $strObservaciones, $intStatus);
                $option = 2;
            }
        }
